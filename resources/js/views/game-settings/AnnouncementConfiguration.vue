<template>
  <div>
    <!-- FILTERS -->
    <div class="box-title">
      <el-button type="primary" icon="el-icon-plus" @click="handlerShowFormAddNewUser">{{ $t('btn.add') }}</el-button>
    </div>
    <!-- TABLE -->
    <el-table ref="multipleTable" v-loading="tables.user.loading" :data="tables.user.list" border fit highlight-current-row style="width: 100%" height="650" @select-all="handlerSelectedAllForDeletes" @selection-change="handlerSelectedChangeForDeletes">
      <!-- -ID- -->
      <el-table-column align="center" :label="$t('table.englishAnnouncementName')" prop="id">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <!-- -USER_NAME- -->
      <el-table-column align="center" :label="$t('table.thaiAnnouncementName')">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <!-- -USER_NIK- -->
      <el-table-column align="center" label="$t('table.')">
        <template slot-scope="scope">
      ...
      <!--  -->
      <el-table-column align="center" :label="$t('placeholders.operationLogStartingTime')">
        <template slot-scope="scope">
          <span>{{ scope.row.create }}</span>
        </template>
      </el-table-column>
      <!--  -->
      <el-table-column align="center" :label="$t('placeholders.operationLogEndTime')">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('table.action')">
        <template>
          <el-button-group class="filter-item" data-id="scope.row.id">
            <el-button type="success" icon="el-icon-plus" @click="handlerShowFormAddNewUser">{{ $t('btn.edit') }}</el-button>
            <el-button type="danger" icon="el-icon-delete" @click="handlerSelectedDeletes">{{ $t('btn.delete') }}</el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- DIALOG ADD AND EDIT USER -->
    <el-dialog title="$t('dialog.titles.addTo') 添加" :visible.sync="dialogs.user.userFormVisible" width="37%">
      <div v-loading="dialogs.user.userLoading" class="form-container">
        <el-form ref="userForm" :rules="dialogs.user.rules" :model="dialogs.user.currentUser" label-position="rigth" label-width="177px" style="width: 500px">
          <!--  -->
          <el-form-item :label="$t('table.englishAnnouncementName')">
            <el-input />
          </el-form-item>
          <!--  -->
          <el-form-item :label="$t('table.thaiAnnouncementName')">
            <el-input />
          </el-form-item>
          <!--  -->
          <el-form-item label="中文公告名称">
            <el-input />
          </el-form-item>
          <!--  -->
          ...
          <!--  -->
          <el-form-item :label="$t('table.type')" prop="code">
          ...
          <!--  -->
          <el-form-item :label="$t('labels.label')" prop="code">
            <el-select placeholder="please select your zone">
              <el-option label="Zone one" value="shanghai" />
              <el-option label="Zone two" value="beijing" />
            </el-select>
          ...
          <!--  -->
          <el-form-item :label="$t('labels.tagResponse')" prop="code">
            <el-select placeholder="please select your zone">
              <el-option label="Zone one" value="shanghai" />
              <el-option label="Zone two" value="beijing" />
            </el-select>
          ...
          <!--  -->
          <el-form-item label="HOT图标" prop="code">
            <el-select placeholder="please select your zone">
              <el-option label="Zone one" value="shanghai" />
              <el-option label="Zone two" value="beijing" />
            </el-select>
          ...
          <!-- START -->
          <el-form-item :label="$t('placeholders.operationLogStartingTime')">
            <el-date-picker type="date" />
          </el-form-item>
          <!-- END -->
          <el-form-item :label="$t('placeholders.operationLogEndTime')">
            <el-date-picker type="date" />
          </el-form-item>
          <!-- -->
          <el-form-item :label="$t('labels.englishImageContent')">
            <el-input />
            <el-button type="success"><i class="el-icon-upload el-icon-right" />&nbsp;{{ $t('btn.uploadPicture') }}</el-button>
          </el-form-item>
          <el-form-item :label="$t('labels.thaiImageContent')">
            <el-input />
            <el-button type="success"><i class="el-icon-upload el-icon-right" />&nbsp;{{ $t('btn.uploadPicture') }}</el-button>
          </el-form-item>
        </el-form>
        ...
        <div slot="footer" class="dialog-footer" align="right">
          <el-button type="success" icon="el-icon-plus" @click="handlerCreateUser">
            {{ $t('btn.save') }}
          </el-button>
          <el-button type="primary" icon="el-icon-close" @click="offUserForm">
            {{ $t('btn.cancel') }}
          </el-button>
        </div>
      </div>
