...
use App\Laravue\Services\ForbiddenIpLoginService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use PragmaRX\Google2FA\Google2FA;

class ForbiddenIpLoginController extends Controller
{
    public function __construct(
        private ResponseFactoryService $responseFactory,
        private Google2FA $google2FA,
        private ForbiddenIpLoginService $forbiddenIpLogin,
    ) {}
    ...
    public function verification(Request $request, User $user): JsonResponse
    {
        ...
        if ($this->google2FA->verify($googleCode, $this->google2FA->getSecret())) {
            ...
            try {
                $this->forbiddenIpLogin->create($ips);
